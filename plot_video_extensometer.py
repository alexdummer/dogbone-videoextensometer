...
def plot_video_extensometer(
    csv_path, video_path=None, frames_dir=None, output_path=None, start_frame=0, end_frame=None
):
    ...
    if start_frame < 0 or end_frame > last_frame or start_frame > end_frame:
        print(
            f"Error: invalid frame range [{start_frame}, {end_frame}] for {len(df)} rows "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "csv_file", type=str, help="Path to the *_extensometer.csv produced by video_extensometer.py"
    )
    parser.add_argument(
        "--video",
        type=str,
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Output PNG path (default: <csv>_overview.png)"
    )
    parser.add_argument(
        "--start-frame",
        type=int,
