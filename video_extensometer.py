...
def run_extensometer_analysis(input_path, output_csv_path, window_size=64, frame_frequency=1, points_file=None,
    ...
    if is_video:
        ...
        if os.path.exists(temp_folder):
            existing_frames = glob.glob(os.path.join(temp_folder, "*.JPG")) + glob.glob(os.path.join(temp_folder, "*.jpg"))
            if len(existing_frames) >= 2:
                need_extraction = False
                print(f"Using {len(existing_frames)} existing frames found in '{temp_folder}'. Skipping extraction.")
    ...
    if start_frame < 0 or end_frame > last_frame or start_frame > end_frame:
        print(f"Error: invalid frame range [{start_frame}, {end_frame}] for {len(img_list)} available frames "
              f"(valid range is [0, {last_frame}]).")
        return
    img_list = img_list[start_frame:end_frame + 1]
    if len(img_list) < 2:
        print(f"Error: Need at least two frames in the selected range [{start_frame}, {end_frame}] to perform analysis.")
        return
    ...
    point_to_process = np.array([
        [np.float32(pts[0][0]), np.float32(pts[0][1])],
        [np.float32(pts[1][0]), np.float32(pts[1][1])]
    ])

    lk_params = dict(winSize=(window_size, window_size), maxLevel=10,
    ...
    for i in range(len(img_list) - 1):
        ...
        final_point, st, err = cv2.calcOpticalFlowPyrLK(img_ref, image_str, point_to_process, None, **lk_params)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Video Extensometer: Track 2 points in a video and compute strain.")
    parser.add_argument("input_source", type=str, help="Path to a video file (.mov/.mp4/.avi) or a directory of *.JPG frames")
    parser.add_argument("--window-size", type=int, default=64, help="Tracking window size in pixels (default: 64)")
    parser.add_argument("--frame-frequency", type=int, default=1, help="Extract every Nth frame if input is a video (default: 1)")
    parser.add_argument("--points-file", type=str, default=None, help="JSON file to save/load the 2 tracked points, to avoid repeated interaction")
    parser.add_argument("--output", type=str, default=None, help="Output CSV path (default: <input>_extensometer.csv next to the input)")
